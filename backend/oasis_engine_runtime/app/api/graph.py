...
# Get logger
logger = get_logger('rekakebijakan.api')
...
# ============== API 1: Upload files and generate an ontology ==============

@graph_bp.route('/ontology/generate', methods=['POST'])
def generate_ontology():
    """
    API 1: Upload files, analyze them, and generate an ontology definition.
    
    Request format: multipart/form-data
    
    Parameters:
        files: One or more uploaded PDF, Markdown, or text files.
...
def _build_graph_impl():
    """
    API 2: Build a graph for a project ID.
    
    Request (JSON):
        {
            "project_id": "proj_xxxx",  // Required; returned by API 1
    ...
    try:
        ...
        # Get configuration
        graph_name = data.get('graph_name', project.name or 'rekakebijakan Graph')
        ...
        def build_task():
            ...
            build_logger = get_logger('rekakebijakan.build')
            try:
                ...
                if resume_existing_batch:
                    ...
                    # Add text; progress_callback accepts (msg, progress_ratio).
                ...
                # waitZepProcessing completed (query eachepisodeofprocessedstate)
                task_manager.update_task(
                    task_id,
                    message=t('progress.waitingZepProcess'),
                    progress=55
