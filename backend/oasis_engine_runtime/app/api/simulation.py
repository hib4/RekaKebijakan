...
Step 2: Fully automated Zep entity filtering and OASIS simulation preparation.
"""
...
logger = get_logger('rekakebijakan.api.simulation')


def _get_default_platform(simulation_id: str) -> str:
    """
    Return the default platform from the simulation configuration.

    Read enable_twitter and enable_reddit from SimulationState and return the
    platform actually used by the simulation instead of hard-coding Reddit.
...
# Interview prompt prefix that requests a direct response without tool calls.
INTERVIEW_PROMPT_PREFIX = "Using your persona, memories, and past actions, reply directly in text without using any tools: "


def optimize_interview_prompt(prompt: str) -> str:
    """
    Add a prefix that asks the agent to answer without calling tools.
...
def get_graph_entities(graph_id: str):
    ...
    Return only nodes matching predefined entity types, not generic Entity nodes.
...
def create_simulation():
    ...
    Notice:max_roundsThe parameters are given byLLMIntelligent generation, no manual settings required
    
    Request (JSON):
        {
            "project_id": "proj_xxxx",      // Required
            "graph_id": "rekakebijakan_xxxx",    // Optional, if not provided then fromprojectget
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    Check conditions:
    1. state.json exist and status for "ready"
    2. Necessary files exist:reddit_profiles.json, twitter_profiles.csv, simulation_config.json
    
    NOTE: Run the script(run_*.py)remain in backend/scripts/ directory, no longer copied to the simulated directory
    ...
    # Required files; scripts remain in backend/scripts/.
    required_files = [
        "state.json",
    ...
    try:
        ...
        # - ready: Ready to run
        # - preparing: if config_generated=True Description completed
        # - running: Running, indicating that the preparation has been completed long ago
        # - completed: The operation is completed, indicating that the preparation has been completed long ago.
        # - stopped: Stopped, indicating that the preparation has been completed long ago
        ...
        if status in prepared_statuses and config_generated:
            ...
            # If the status ispreparingBut the file is completed and the auto-update status isready
            if status == "preparing":
                try:
            ...
            return False, {
                "reason": f"The status is not in the prepared list orconfig_generatedforfalse: status={status}, config_generated={config_generated}",
                "status": status,
                "config_generated": config_generated
            }
...
def prepare_simulation():
    """
    Prepare simulation environment (asynchronous tasks,LLMIntelligent generation of all parameters)
    
    This is a long-running operation; the API returns a task_id immediately.
    ...
    step:
    1. Check whether preparations have been completed
    2. fromZepGraph reads and filters entities
    3. Generate an OASIS Agent Profile for each entity, with retries.
    4. LLMIntelligent generation of simulation configuration (with retry mechanism)
    ...
    Request (JSON):
        {
            ...
            "use_llm_for_profiles": true,                 // Optional, whether to useLLMGenerate character
            "parallel_profile_count": 5,                  // Optional; parallel profile count, default: 5
            "force_regenerate": false                     // Optional, force regeneration, defaultfalse
        }
    ...
    return:
        {
            ...
            "data": {
                "simulation_id": "sim_xxxx",
                "task_id": "task_xxxx",           // Return when new task
                "status": "preparing|ready",
                "message": "The preparation task has been started|Preparatory work has been completed",
                "already_prepared": true|false    // Are you ready?
            }
        }
    """
    import threading
    ...
    try:
        ...
        logger.info(f"Start processing /prepare ask: simulation_id={simulation_id}, force_regenerate={force_regenerate}")
        
        # Check whether it is ready (to avoid repeated generation)
        if not force_regenerate:
            logger.debug(f"Check the simulation {simulation_id} Are you ready?...")
            is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
            logger.debug(f"Check results: is_prepared={is_prepared}, prepare_info={prepare_info}")
            if is_prepared:
                logger.info(f"simulation {simulation_id} Prepared, skip repeated generation")
                return jsonify({
                    "success": True,
                    "data": {
                ...
                logger.info(f"simulation {simulation_id} The preparation is not completed and the preparation task will be started.")
        ...
        # This lets the frontend obtain the expected agent count immediately.
        try:
            logger.info(f"Get the number of entities synchronously: graph_id={state.graph_id}")
            reader = ZepEntityReader()
...
def get_prepare_status():
    ...
    1. passtask_idQuery the progress of ongoing tasks
    2. passsimulation_idCheck whether preparations have been completed
    
    Request (JSON):
        {
            "task_id": "task_xxxx",          // Optional,preparereturnedtask_id
            "simulation_id": "sim_xxxx"      // Optional; simulation ID used to check preparation
        }
    ...
    return:
        {
            ...
            "data": {
                ...
                "progress": 45,
                "message": "...",
                "already_prepared": true|false,  // Are preparations completed?
    ...
    try:
        ...
        # If simulation_id is provided, check whether it is ready first.
        if simulation_id:
            is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
            if is_prepared:
                return jsonify({
        ...
        # Return an error when task_id is absent.
        if not task_id:
            if simulation_id:
                # havesimulation_idbut not ready yet
                return jsonify({
                    "success": True,
                    "data": {
                        "simulation_id": simulation_id,
        ...
        if not task:
            # If the task is absent, use simulation_id to check readiness.
            if simulation_id:
                is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
                if is_prepared:
                    return jsonify({
...
def _get_report_id_for_simulation(simulation_id: str) -> str:
    """
    get simulation The corresponding latest report_id
    
    Search the reports directory for reports matching simulation_id.
    If there are multiple, return the latest one (press created_at Sort)
    ...
    # __file__ is app/api/simulation.py, two levels below backend/.
...
def get_simulation_history():
    ...
    try:
        ...
        # Enhanced simulation data, only from Simulation file reading
        ...
        for sim in simulations:
            ...
            if run_state:
                ...
                # Use the configured total_rounds, or fall back to the recommendation.
            ...
            # Get the file list of the associated project (up to3indivual)
            ...
            # Find the latest report associated with the simulation.
...
def get_simulation_profiles(simulation_id: str):
    ...
    try:
        ...
        logger.error(f"getProfilefail: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def get_simulation_profiles_realtime(simulation_id: str):
    """
    Get simulation agent profiles in real time while they are generated.
    
    and /profiles The difference between interfaces:
    - Read the file directly without going through SimulationManager
    - Suitable for real-time viewing during the build process
    ...
    try:
        ...
        # Check if it is being generated (via state.json judge)
        is_generating = False
        total_expected = None
        status = None
        error = None
...
def get_simulation_config_realtime(simulation_id: str):
    ...
    and /config The difference between interfaces:
    - Read the file directly without going through SimulationManager
    - Suitable for real-time viewing during the build process
    - Return additional metadata (such as file modification time, whether it is being generated, etc.)
    ...
    return:
        {
            ...
            "data": {
                ...
                "config": {...}  // Configuration content (if present)
            }
        }
    """
    import json
...
def get_simulation_config(simulation_id: str):
    """
    Get simulation configuration (LLMSmartly generated complete configuration)
    
    Return contains:
        - time_config: Time configuration (simulation duration, rounds, peak/low period)
        - agent_configs: Per-agent activity, posting frequency, stance, and related settings.
        - event_config: Event configuration (initial post, hot topics)
        - platform_configs: Platform configuration
        - generation_reasoning: LLMConfiguration reasoning description of
    """
    try:
        manager = SimulationManager()
...
def download_simulation_script(script_name: str):
    """
    Download a shared simulation script from backend/scripts/.
...
# ============== ProfileGenerate interface (for independent use) ==============
...
def generate_profiles():
    ...
    Request (JSON):
        {
            "graph_id": "rekakebijakan_xxxx",     // Required
...
def start_simulation():
    ...
    Request (JSON):
        {
            ...
            "enable_graph_memory_update": false,   // Optional; write agent activity to Zep graph memory
    ...
    about force parameter:
        ...
        - Cleanup content includes:run_state.json, actions.jsonl, simulation.log wait
        - Configuration and profile files are retained.
        - Suitable for scenarios where the simulation needs to be re-run

    About enable_graph_memory_update:
        - When enabled, agent activity is written to the Zep graph in real time.
        - This allows the graph to"remember"Simulate the process for subsequent analysis orAIdialogue
        - Projects that require simulation association have valid graph_id
        - Adopt batch update mechanism to reduceAPINumber of calls
    ...
    try:
        ...
        if state.status != SimulationStatus.READY:
            ...
            if is_prepared:
                ...
                if needs_finalization:
                    ...
                    logger.info(f"Force mode: finish old simulation first and terminate {simulation_id}")
                    try:
                ...
                # The process does not exist or has ended, and the reset status is ready
                logger.info(f"Simulation {simulation_id} is prepared; resetting status to ready (previous status: {state.status.value})")
        ...
        # Get the graph ID used for graph memory updates.
        graph_id = None
        ...
        with graph_guard:
            if enable_graph_memory_update:
                ...
                logger.info(
                    "Enable graph memory updates: simulation_id=%s, graph_id=%s",
                    simulation_id,
                    graph_id,
                )

            # Keep graph_guard while starting a graph-writing simulation, until the updater
            # claim All release of process resources is completed.
...
def get_run_status_detail(simulation_id: str):
    ...
    Queryparameter:
        platform: Filter by twitter or reddit (optional).
    ...
    try:
        ...
        # Get the action of the current round (recent_actions Only the latest round is shown)
...
def get_agent_stats(simulation_id: str):
    ...
    Used for frontend activity rankings and action distributions.
    """
    try:
...
def get_simulation_posts(simulation_id: str):
    ...
    Return list of posts (fromSQLitedatabase reading)
    """
    try:
        platform = request.args.get('platform') or _get_default_platform(simulation_id)
...
def get_simulation_comments(simulation_id: str):
    """
    Get comments in simulation

    Queryparameter:
        platform: Platform type, selected from the simulation configuration by default.
...
def interview_agent():
    ...
    Request (JSON):
        {
            ...
            "prompt": "What do you think about this matter?",  // Required, interview questions
            "platform": "twitter",             // Optional; twitter or reddit
                                               // When not specified: Dual-platform simulation interviews two platforms at the same time
            "timeout": 60                      // Optional; timeout in seconds, default: 60
        }

    Return value when platform is omitted in dual-platform mode:
        {
            "success": true,
            "data": {
                "agent_id": 0,
    ...
    Return value when platform is specified:
        {
            "success": true,
            "data": {
                "agent_id": 0,
    ...
    try:
        ...
        # Prefix the prompt to discourage tool calls.
...
def interview_agents_batch():
    ...
    Request (JSON):
        {
            ...
            "interviews": [                    // Required, interview list
                ...
                {
                    "agent_id": 1,
                    "prompt": "What do you think about B?"  // Uses the default platform
                }
            ],
            "platform": "reddit",              // Optional, default platform (specified by eachplatformcover)
                                               // If omitted, interviews each agent on both platforms
    ...
    try:
        ...
        for i, interview in enumerate(interviews):
            ...
            # Validate each item's platform when present.
        ...
        # Prefix each prompt to discourage tool calls.
        ...
        logger.error(f"batchInterviewfail: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def interview_all_agents():
    ...
    Request (JSON):
        {
            "simulation_id": "sim_xxxx",            // Required; simulation ID
            "prompt": "What's your overall opinion on this matter?",  // Required; shared question
            "platform": "reddit",                   // Optional; twitter or reddit
                                                    // If omitted, interviews each agent on both platforms
    ...
    try:
        ...
        logger.error(f"overall situationInterviewfail: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def get_interview_history():
    ...
    Read all from simulated databaseInterviewRecord
    ...
    Request (JSON):
        {
            ...
            "limit": 100                   // Optional; maximum results, default: 100
        }
...
def get_env_status():
    ...
    Check if the simulated environment is alive (can receiveInterviewOrder)
    ...
    return:
        {
            ...
            "data": {
                ...
                "message": "The environment is running and can receiveInterviewOrder"
            }
        }
    """
    try:
...
def close_simulation_env():
    ...
    NOTE: This is different from /stop interface,/stop will forcefully terminate the process.
