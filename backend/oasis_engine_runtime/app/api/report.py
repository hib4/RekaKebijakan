...
logger = get_logger('rekakebijakan.api.report')
...
def generate_report():
    ...
    try:
        ...
        # Generate report_id in advance so it can be returned immediately.
        import uuid
...
def get_generate_status():
    ...
    try:
        ...
        # If simulation_id is provided, check for a completed report first.
        if simulation_id:
...
def get_report_by_simulation(simulation_id: str):
    """
    According to simulationIDGet report
...
def list_reports():
    ...
    Queryparameter:
        simulation_id: Press AnalogIDFilter (optional)
    ...
    try:
        ...
        logger.error(f"Listing reports failed: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def download_report(report_id: str):
    """
    Download report (MarkdownFormat)
    
    returnMarkdowndocument
    """
    try:
        report = ReportManager.get_report(report_id)
        ...
        if not os.path.exists(md_path):
            # ifMDThe file does not exist and a temporary file is generated.
            import tempfile
...
def chat_with_report_agent():
    ...
    The Report Agent can use search tools to answer questions.
    ...
    try:
        ...
        # Create the agent and start the conversation.
        agent = ReportAgent(
            graph_id=graph_id,
            simulation_id=simulation_id,
            simulation_requirement=simulation_requirement
...
def get_single_section(report_id: str, section_index: int):
    ...
    try:
        ...
        logger.error(f"Failed to obtain chapter content: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def check_report_status(simulation_id: str):
    ...
    Used by the front end to determine whether to unlockInterviewFunction
    ...
    try:
        ...
        # Unlocked only after report is completedinterview
...
def get_agent_log(report_id: str):
    ...
    - Reporting begins, planning begins/Finish
    - The beginning of each chapter, tool call,LLMresponse, completion
    - Report completion or failure
    
    Queryparameter:
        from_line: First line to read (optional; default: 0, for incremental reads).
...
def get_console_log(report_id: str):
    """
    Get the Report Agent console output.

    This endpoint returns real-time, plain-text console logs such as INFO and
    WARNING messages. Unlike the agent-log endpoint, it does not return
    ...
    Returns:
        {
            ...
            "data": {
                "logs": [
                    "[19:46:14] INFO: Search completed: turn up 15 relevant facts",
