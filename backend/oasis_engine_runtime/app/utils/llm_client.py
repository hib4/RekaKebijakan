"""
LLM client wrapper.
Uses the OpenAI request format consistently.
"""
...
class LLMClient:
    """Client for interacting with the configured LLM."""
    ...
    def _create_completion(
        ...
        *,
        ...
        logger.info(
            "LLM request started model=%s messages=%d json_mode=%s",
        ...
        try:
            ...
            logger.exception(
                "LLM request failed model=%s duration_ms=%d",
                self.model, int((time.monotonic() - started_at) * 1000),
            )
            raise
        logger.info(
    ...
    def chat_json(
        ...
        Args:
            ...
            temperature: Sampling temperature
            max_tokens: Maximum number of tokens
            max_attempts: Content generation attempts, excluding one explicit JSON-mode capability fallback
