...
Dynamically add simulated agent activity to a Zep graph.
"""
...
logger = get_logger('rekakebijakan.zep_graph_memory_updater')
...
class AgentActivity:
    ...
    def to_episode_text(self) -> str:
        """
        Convert an activity into natural-language text for Zep extraction.

        Simulation-specific prefixes are omitted to avoid misleading graph updates.
        """
        # Generate a description for the action type.
        action_descriptions = {
    ...
    def _describe_like_post(self) -> str:
        """Describe liking a post with its text and author."""
    ...
    def _describe_dislike_post(self) -> str:
        """Describe disliking a post with its text and author."""
        post_content = self.action_args.get("post_content", "")
        post_author = self.action_args.get("post_author_name", "")
    ...
    def _describe_repost(self) -> str:
        """Describe reposting with the original text and author."""
    ...
    def _describe_quote_post(self) -> str:
        """Describe quoting a post with its text, author, and commentary."""
        original_content = self.action_args.get("original_content", "")
        original_author = self.action_args.get("original_author_name", "")
    ...
    def _describe_follow(self) -> str:
        """Describe following a named user."""
    ...
    def _describe_create_comment(self) -> str:
        """Describe a comment with its content and post context."""
        content = self.action_args.get("content", "")
        post_content = self.action_args.get("post_content", "")
        post_author = self.action_args.get("post_author_name", "")
    ...
    def _describe_like_comment(self) -> str:
        """Describe liking a comment with its text and author."""
    ...
    def _describe_dislike_comment(self) -> str:
        """Describe disliking a comment with its text and author."""
        comment_content = self.action_args.get("comment_content", "")
        comment_author = self.action_args.get("comment_author_name", "")
    ...
    def _describe_generic(self) -> str:
        # Use a generic description for unknown action types.
...
class ZepGraphMemoryUpdater:
    """
    Update Zep graph memory from simulated agent activity.

    Activities are grouped by platform and sent after BATCH_SIZE records.
    Action arguments retain complete post, comment, and user context.
    """
    
    # Number of accumulated records sent per platform batch.
    BATCH_SIZE = 5
    
    # Platform names used in console output.
    PLATFORM_DISPLAY_NAMES = {
        'twitter': 'Twitter',
        'reddit': 'Reddit',
    }
    
    # Delay between sends to avoid excessive request rates.
    SEND_INTERVAL = 0.5
    ...
    def __init__(
        ...
        Args:
            ...
            api_key: Optional Zep API key; defaults to configuration.
        """
        ...
        self._skipped_count = 0     # Filtered DO_NOTHING activities.
    ...
    def add_activity(self, activity: AgentActivity):
        """
        Add a meaningful agent activity to the queue.

        Supported activities include posts, comments, quotes, searches, likes,
        dislikes, reposts, follows, mutes, and comment reactions. ``action_args``
        contains complete context such as original text and usernames.
    ...
    def _worker_loop(self, locale: str = 'en'):
        ...
        while self._running or not self._activity_queue.empty():
            try:
                # Wait up to one second for an activity.
                try:
                    ...
                    with self._buffer_lock:
                        ...
                        # Check whether the platform reached its batch size.
    ...
    def _build_episode_payloads(
        ...
        for activity in activities:
            ...
            if len(text) > self.MAX_EPISODE_CHARS:
                marker = "... [truncated by rekakebijakan]"
    ...
    def _send_batch_activities(
        ...
        *,
        ...
        """
        Send activities to the Zep graph as combined text.
        ...
        for payload_activities, combined_text in self._build_episode_payloads(activities):
            ...
            try:
                episode = self.client.graph.add(
                    ...
                    source_description="rekakebijakan simulation activity batch",
                    metadata={
                        "source": "rekakebijakan_simulation",
                ...
                logger.error(f"Zep batch send failed; non-idempotent write was not replayed: {e}")
    ...
    def _flush_remaining(self, *, deadline: float | None = None):
        """Send remaining queued and buffered activities."""
        # Move remaining queued activities into buffers first.
        ...
        for platform in list(self._platform_buffers):
            ...
            if not buffer:
                continue
            display_name = self._get_platform_display_name(platform)
            logger.info(f"Sending {len(buffer)} remaining activities for {display_name}")
            if deadline is not None and time.time() >= deadline:
                raise TimeoutError(
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_activities": self._total_activities,  # Activities added to queue.
            ...
            "skipped_count": self._skipped_count,        # Filtered DO_NOTHING activities.
...
class ZepGraphMemoryManager:
    ...
    def create_updater(cls, simulation_id: str, graph_id: str) -> ZepGraphMemoryUpdater:
        ...
        """
        with cls._lock:
            # Stop an existing updater first.
    ...
    def stop_all(cls):
        ...
        if errors:
            ...
            raise RuntimeError(f"Some graph updaters did not stop completely: {details}")
