...
Runs simulations in the background, records agent actions, and supports live monitoring.
"""
...
logger = get_logger('rekakebijakan.simulation_runner')

# Whether the cleanup handler has been registered.
...
class SimulationRunState:
    ...
    # Per-platform rounds and simulated time for parallel display.
    ...
    # Recent actions for live frontend display.
    ...
    def to_detail_dict(self) -> Dict[str, Any]:
        """Return detailed state including recent actions."""
        result = self.to_dict()
...
class SimulationRunner:
    ...
    def _sync_simulation_status(
        ...
        try:
            ...
            # ingestion drain.
            logger.error(
                "Failed to synchronize simulation status: simulation_id=%s, status=%s, error=%s",
                simulation_id,
                runner_status.value,
                sync_error,
            )
    ...
    def start_simulation(
        ...
        max_rounds: int = None,  # Optional cap for long simulations.
        enable_graph_memory_update: bool = False,  # Whether to update the Zep graph.
        graph_id: str = None  # Required Zep graph ID when graph updates are enabled.
    ) -> SimulationRunState:
        """
        Start a simulation.
        ...
        Args:
            ...
            max_rounds: Optional maximum number of simulation rounds.
            enable_graph_memory_update: Whether to add agent activity to the Zep graph.
        ...
        # Apply the optional round limit.
        if max_rounds is not None and max_rounds > 0:
            original_rounds = total_rounds
            total_rounds = min(total_rounds, max_rounds)
            if total_rounds < original_rounds:
                logger.info(f"Round count limited: {original_rounds} -> {total_rounds} (max_rounds={max_rounds})")
        ...
        with cls._finalization_lock(simulation_id):
            ...
            if (
                ...
                raise ValueError(f"Simulation is already running or being finalized: {simulation_id}")
            cls._save_run_state(state)
        
        # Create the graph-memory updater when enabled.
        ...
        # Select the script from backend/scripts/.
        if platform == "twitter":
        ...
        if not os.path.exists(script_path):
            ...
            if cleanup_error is not None:
                state.error += f"; failed to clean up Zep graph writes: {cleanup_error}"
            with cls._finalization_lock(simulation_id):
                cls._save_run_state(state)
                cls._sync_simulation_status(
                    simulation_id,
        ...
        try:
            ...
            cmd = [
                ...
                script_path,
                "--config", config_path,  # Absolute configuration path.
            ]
            
            # Add the optional round limit.
            if max_rounds is not None and max_rounds > 0:
                cmd.extend(["--max-rounds", str(max_rounds)])
            
            # Use a log file to prevent full stdout/stderr pipes from blocking.
            main_log_path = os.path.join(sim_dir, "simulation.log")
            main_log_file = open(main_log_path, 'w', encoding='utf-8')
            
            # Force UTF-8 in the child process, especially on Windows.
            # This supports libraries such as OASIS that open files without an encoding.
            env = os.environ.copy()
            env['PYTHONUTF8'] = '1'  # Make open() default to UTF-8 on Python 3.7+.
            env['PYTHONIOENCODING'] = 'utf-8'  # Use UTF-8 for stdout/stderr.
            
            # Use the simulation directory for generated databases and other files.
            # Create a process group so os.killpg can terminate all child processes.
            process = subprocess.Popen(
                cmd,
                cwd=sim_dir,
                stdout=main_log_file,
                stderr=subprocess.STDOUT,  # Write stderr to the same file.
                text=True,
                encoding='utf-8',  # Explicit encoding.
                bufsize=1,
                env=env,  # Pass the UTF-8 environment.
                start_new_session=True,  # Create a process group for reliable shutdown.
            )
            ...
            if cls._graph_memory_enabled.get(simulation_id, False):
                try:
                    ...
                    cleanup_errors.append(f"Failed to clean up Zep graph writes: {error}")
            state.runner_status = RunnerStatus.FAILED
            state.twitter_running = False
            state.reddit_running = False
            state.error = str(e)
    ...
    def _monitor_simulation(cls, simulation_id: str, locale: str = 'id'):
        ...
        # Separate action logs for each platform.
        ...
        try:
            ...
            # Read each log once more after the process exits.
            if os.path.exists(twitter_actions_log):
                ...
                if state.runner_status not in {
                    ...
                    if cls._graph_memory_enabled.get(simulation_id, False):
                        ...
                        try:
                            ...
                            cls._graph_memory_enabled.pop(simulation_id, None)
                            logger.info(
                                "Graph-memory updates stopped: simulation_id=%s",
                                simulation_id,
                            )
                        except Exception as error:
                    ...
                    if line:
                        try:
                            ...
                            if "event_type" in action_data:
                                ...
                                # Mark the platform complete on simulation_end.
                                if event_type == "simulation_end":
                                    ...
                                    # A single-platform run checks only that platform;
                                    # a dual-platform run requires both.
                                ...
                                # Update round information from round_end events.
                                    ...
                                    # Use the maximum platform round as the overall round.
                                    if round_num > state.current_round:
                                        state.current_round = round_num
                                    # Use the maximum platform time as the overall time.
                            ...
                            # Send activity to Zep when graph-memory updates are enabled.
                            if graph_updater:
    ...
    def _check_all_platforms_completed(cls, state: SimulationRunState) -> bool:
        """
        Check whether all enabled platforms completed their simulations.
        ...
        # Detect enabled platforms by file existence.
        twitter_enabled = os.path.exists(twitter_log)
        reddit_enabled = os.path.exists(reddit_log)
        
        # Return False if an enabled platform is incomplete.
    ...
    def _terminate_process(cls, process: subprocess.Popen, simulation_id: str, timeout: int = 10):
        ...
        Args:
            ...
            simulation_id: Simulation ID used in logs.
            timeout: Process exit timeout in seconds.
        """
        if IS_WINDOWS:
            # Windows: terminate the process tree with taskkill.
            # /F forces termination; /T includes child processes.
            ...
            # start_new_session=True makes the group ID equal the main process PID.
            ...
            try:
                ...
                # Send SIGKILL if the timeout expires.
        ...
        if (
            ...
            if monitor.is_alive():
                ...
                raise SimulationStopPending(
                    f"Simulation is still stopping; graph writes did not finish within {wait_timeout:.0f}s"
                )
        else:
    ...
    def _read_actions_from_file(
        ...
        Args:
            ...
            default_platform: Platform used when an action has no platform field.
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                try:
                    ...
                    # Skip records without agent_id because they are not agent actions.
                    if "agent_id" not in data:
                        continue
                    
                    # Prefer the record's platform, then use the default.
    ...
    def get_all_actions(
        ...
        """
        Return complete action history across platforms without pagination.
        ...
        Returns:
            Complete action list sorted newest first.
        """
        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
        actions = []
        
        # Read Twitter actions and infer the platform from the path.
        twitter_actions_log = os.path.join(sim_dir, "twitter", "actions.jsonl")
        if not platform or platform == "twitter":
            actions.extend(cls._read_actions_from_file(
                twitter_actions_log,
                default_platform="twitter",  # Fill the platform field.
                platform_filter=platform,
                agent_id=agent_id, 
                round_num=round_num
            ))
        
        # Read Reddit actions and infer the platform from the path.
        reddit_actions_log = os.path.join(sim_dir, "reddit", "actions.jsonl")
        if not platform or platform == "reddit":
            actions.extend(cls._read_actions_from_file(
                reddit_actions_log,
                default_platform="reddit",  # Fill the platform field.
                platform_filter=platform,
                agent_id=agent_id,
                round_num=round_num
            ))
        
        # Fall back to the legacy combined file when platform files have no actions.
        if not actions:
            actions_log = os.path.join(sim_dir, "actions.jsonl")
            actions = cls._read_actions_from_file(
                actions_log,
                default_platform=None,  # Legacy records should contain platform.
                platform_filter=platform,
                agent_id=agent_id,
                round_num=round_num
            )
    ...
    def get_timeline(
        ...
        """
        Return the simulation timeline grouped by round.
    ...
    def get_agent_stats(cls, simulation_id: str) -> List[Dict[str, Any]]:
        """
        Return statistics for each agent.
    ...
    def cleanup_simulation_logs(cls, simulation_id: str) -> Dict[str, Any]:
        """
        Clean simulation run logs to force a fresh run.
        ...
        Configuration and profile files are preserved.
        ...
        if not os.path.exists(sim_dir):
            return {"success": True, "message": "Simulation directory does not exist; no cleanup is needed"}
        
        cleaned_files = []
        errors = []
        
        # Files to delete, including databases.
        files_to_delete = [
            "run_state.json",
            "simulation.log",
            "stdout.log",
    ...
    def cleanup_all_simulations(cls):
        ...
        Called during server shutdown to terminate all child processes.
        """
        # Prevent duplicate cleanup.
        ...
        for simulation_id in simulation_ids:
            try:
                ...
                cleanup_failed = True
                logger.error(
                    "Simulation cleanup failed; retaining state for retry: simulation_id=%s, error=%s",
                    simulation_id,
                    error,
                )
        ...
        if cleanup_failed:
            ...
            logger.error("Some simulations were not cleaned up safely")
        else:
            logger.info("Simulation process and graph-write cleanup completed")
    ...
    def register_cleanup(cls):
        ...
        Called when Flask starts to clean up simulations during server shutdown.
        """
        ...
        # In Flask debug mode, register only in the reloader child that runs the app.
        # WERKZEUG_RUN_MAIN=true identifies the reloader child.
        # Without debug mode the variable is absent and handlers must still be registered.
        is_reloader_process = os.environ.get('WERKZEUG_RUN_MAIN') == 'true'
        is_debug_mode = os.environ.get('FLASK_DEBUG') == '1' or os.environ.get('WERKZEUG_RUN_MAIN') is not None
        
        # Register in the reloader child during debug mode, and always otherwise.
        if is_debug_mode and not is_reloader_process:
            _cleanup_registered = True  # Prevent another registration attempt.
            return
        
        # Preserve existing signal handlers.
        original_sigint = signal.getsignal(signal.SIGINT)
        original_sigterm = signal.getsignal(signal.SIGTERM)
        # SIGHUP exists on Unix systems but not Windows.
        ...
        def cleanup_handler(signum=None, frame=None):
            """Clean simulation processes, then invoke the original signal handler."""
            # Log only when resources need cleanup.
            if cls._processes or cls._graph_memory_enabled:
                logger.info(f"Received signal {signum}; starting cleanup...")
            cls.cleanup_all_simulations()
            
            # Invoke the original handler so Flask exits normally.
            if signum == signal.SIGINT and callable(original_sigint):
                ...
                if callable(original_sighup):
                    ...
                    # Default to a normal exit.
                    sys.exit(0)
            else:
                # Use default behavior when the original handler is not callable.
                raise KeyboardInterrupt
        
        # Register atexit as a fallback.
        atexit.register(cls.cleanup_all_simulations)
        
        # Register signal handlers in the main thread only.
        try:
            # SIGTERM: default signal used by kill.
            ...
            # Outside the main thread, only atexit is available.
            logger.warning("Cannot register signal handlers outside the main thread; using atexit only")
    ...
    def get_running_simulations(cls) -> List[str]:
        """
        Return IDs of all running simulations.
        """
        running = []
    ...
    def check_env_alive(cls, simulation_id: str) -> bool:
        """
        Check whether the simulation environment can receive interview commands.
    ...
    def get_env_status_detail(cls, simulation_id: str) -> Dict[str, Any]:
        """
        Return detailed simulation environment status.

        Args:
            simulation_id: Simulation ID.

        Returns:
            Status details containing status, twitter_available, reddit_available, and timestamp.
        """
        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
    ...
    def interview_agent(
        ...
        Args:
            ...
            platform: Optional target platform.
                ...
                - None: Interview both platforms and return combined results.
        ...
        Raises:
            ...
            TimeoutError: The response timed out.
        """
        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
        if not os.path.exists(sim_dir):
            raise ValueError(f"Simulation does not exist: {simulation_id}")
        ...
        if not ipc_client.check_env_alive():
            raise ValueError(f"Simulation environment is not running or is closed; cannot interview: {simulation_id}")
    ...
    def interview_agents_batch(
        ...
        Args:
            simulation_id: Simulation ID.
            interviews: Entries containing agent_id, prompt, and an optional platform.
            platform: Optional default platform, overridden by each entry's platform.
                - "twitter": Interview Twitter by default.
                - "reddit": Interview Reddit by default.
    ...
    def interview_all_agents(
        ...
        """
        Interview all agents globally.

        Uses the same question for every agent in the simulation.

        Args:
            simulation_id: Simulation ID.
            prompt: Question used for every agent.
            platform: Optional target platform.
                - "twitter": Interview Twitter only.
                - "reddit": Interview Reddit only.
                - None: Interview each agent on both platforms.
        ...
        Returns:
            Global interview result.
        """
        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
        if not os.path.exists(sim_dir):
            raise ValueError(f"Simulation does not exist: {simulation_id}")

        # Read all agent information from the configuration.
        config_path = os.path.join(sim_dir, "simulation_config.json")
        if not os.path.exists(config_path):
        ...
        if not agent_configs:
            raise ValueError(f"Simulation configuration contains no agents: {simulation_id}")
    ...
    def close_simulation_env(
        ...
        Sends a close command so the simulation gracefully exits command-wait mode.
        ...
        logger.info(f"Sending environment close command: simulation_id={simulation_id}")
        ...
        try:
            ...
            # The timeout may occur because the environment is closing.
            return {
                "success": True,
                "message": "Environment close command sent; the response timed out and the environment may be closing"
            }
    ...
    def _get_interview_history_from_db(
        ...
        """Return interview history from one database."""
        import sqlite3
    ...
    def get_interview_history(
        ...
        """
        Return interview history from the databases.
        ...
        # Select platforms to query.
        if platform in ("reddit", "twitter"):
            platforms = [platform]
        else:
            # Query both platforms when none is specified.
        ...
        # Apply the total limit when querying multiple platforms.
