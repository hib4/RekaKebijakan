...
API 2: Build a standalone graph with the Zep API.
"""
...
class GraphBuilderService:
    """
    Graph construction service that builds knowledge graphs through the Zep API.
    """
    ...
    def build_graph_async(
        ...
        graph_name: str = "rekakebijakan Graph",
        chunk_size: int = 500,
        ...
        Args:
            text: Input text.
            ontology: Ontology definition returned by API 1.
        ...
        # Run construction in a background thread.
    ...
    def _build_graph_worker(
        ...
        try:
            ...
            # 3. Text chunks were created and validated before the Cloud mutation.
            self.task_manager.update_task(
                task_id,
                progress=20,
    ...
    def create_graph(
        ...
        *,
        ...
        graph_id = graph_id or f"rekakebijakan_{uuid.uuid4().hex[:16]}"
        ...
        try:
            self.client.graph.create(
                graph_id=graph_id,
                name=name,
                description="rekakebijakan Social Simulation Graph"
            )
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            while True:
                ...
                for batch in getattr(page, "batches", None) or []:
                    ...
                    if (
                        metadata.get("rekakebijakan_operation_id") == operation_id
    ...
    def set_ontology(self, graph_id: str, ontology: Dict[str, Any]):
        ...
        # Suppress the Pydantic v2 Field(default=None) warning required by the
        # Zep SDK when dynamically creating classes.
        warnings.filterwarnings('ignore', category=UserWarning, module='pydantic')
        
        def safe_attr_name(attr_name: str) -> str:
            """Convert a reserved attribute name to a safe name."""
            if attr_name.lower() in RESERVED_ONTOLOGY_ATTRIBUTE_NAMES:
                return f"entity_{attr_name}"
            return attr_name
        
        # Create entity types dynamically.
        entity_types = {}
        for entity_def in ontology.get("entity_types", [])[:MAX_ONTOLOGY_TYPES]:
            name = entity_def["name"]
            description = entity_def.get("description", f"A {name} entity.")
            
            # Create the attribute dictionary and Pydantic v2 annotations.
            ...
            for normalized in normalize_ontology_attributes(
                ...
                attr_name = safe_attr_name(normalized["name"])  # Use a safe name.
        ...
        # Create edge types dynamically.
        edge_definitions = {}
        for edge_def in ontology.get("edge_types", [])[:MAX_ONTOLOGY_TYPES]:
            name = edge_def["name"]
            description = edge_def.get("description", f"A {name} relationship.")
            
            # Create the attribute dictionary and type annotations.
            attrs = {"__doc__": description}
            annotations = {}
            ...
            for normalized in normalize_ontology_attributes(
                ...
                # The Zep API requires a Field description.
                attrs[attr_name] = Field(description=attr_desc, default=None)
                annotations[attr_name] = Optional[str]  # Edge attributes use strings.
        ...
        # Set the ontology through the Zep API.
        ...
        for i in range(0, total_chunks, batch_size):
            ...
            items = [
                BatchAddItem(
                    ...
                    data=chunk,
                    data_type="text",
                    source_description="rekakebijakan source document chunk",
                    metadata={
                        "rekakebijakan_operation_id": operation_id,
    ...
    def _wait_for_episodes(
        ...
        """Wait for all episodes by polling each episode's processed state."""
        ...
        while pending_episodes:
            ...
            if pending_episodes:
                time.sleep(3)  # Poll every three seconds.
    ...
    def get_graph_data(self, graph_id: str) -> Dict[str, Any]:
        ...
        Returns:
            A dictionary containing nodes, edges, temporal data, attributes, and details.
        """
        nodes = fetch_all_nodes(self.client, graph_id)
        edges = fetch_all_edges(self.client, graph_id)
