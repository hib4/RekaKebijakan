"""Generate OASIS agent profiles from entities in a Zep graph.

Enhancements:
1. Enrich node information with additional Zep retrieval.
2. Generate detailed personas with improved prompts.
...
class OasisAgentProfile:
    ...
    def to_reddit_format(self) -> Dict[str, Any]:
        ...
        profile = {
            ...
            "username": self.user_name,  # OASIS requires "username" without an underscore
            "name": self.name,
            "bio": self.bio,
        ...
        # Add optional persona information
        if self.age:
    ...
    def to_dict(self) -> Dict[str, Any]:
        """Convert to the complete dictionary format."""
        return {
            "user_id": self.user_id,
...
class OasisProfileGenerator:
    ...
    The generator enriches context through Zep graph retrieval, creates
    detailed personas, and distinguishes individuals from groups.
    """
    ...
    # Individual entity types that need a specific persona
    INDIVIDUAL_ENTITY_TYPES = [
        "student", "alumni", "professor", "person", "publicfigure", 
        "expert", "faculty", "official", "journalist", "activist"
    ]
    
    # Group and institution types that need a representative persona
    GROUP_ENTITY_TYPES = [
    ...
    def generate_profile_from_entity(
        ...
        Returns:
            The generated OASIS agent profile.
        """
        ...
        if use_llm:
            ...
            # Generate a basic persona with rules
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        """Retrieve rich entity information using Zep graph search.

        Zep does not provide a built-in hybrid search endpoint, so edges and
        nodes are searched concurrently and their results are merged.

        Args:
            entity: Entity node object.

        Returns:
            A dictionary containing facts, node summaries, and context.
        """
        ...
        # A graph ID is required for search
        ...
        try:
            # Search edges and nodes concurrently
            ...
            logger.info(f"Zep retrieval completed for {entity_name}: {len(results['facts'])} facts and {len(results['node_summaries'])} related nodes")
    ...
    def _build_entity_context(self, entity: EntityNode) -> str:
        ...
        Includes the entity's edge facts, related-node details, and
        additional information retrieved from Zep.
        """
        context_parts = []
        
        # 1. Add entity attributes
        ...
        if entity.related_edges:
            relationships = []
            for edge in entity.related_edges:  # Do not limit the number of edges
                fact = edge.get("fact", "")
        ...
        if entity.related_nodes:
            related_info = []
            for node in entity.related_nodes:  # Do not limit the number of nodes
        ...
        # 4. Retrieve richer information from Zep
        zep_results = self._search_zep_for_entity(entity)
        
        if zep_results.get("facts"):
            # Exclude facts already present
    ...
    def _generate_profile_with_llm(
        ...
        Individuals receive a specific personal profile, while groups and
        institutions receive a representative account profile.
        """
        ...
        # Retry until generation succeeds or the attempt limit is reached
        max_attempts = 3
        last_error = None
        ...
        for attempt in range(max_attempts):
            try:
                response = create_chat_completion(
                    ...
                    # Leave max_tokens unset so the LLM can produce a complete response
                )
        ...
        logger.warning(f"LLM persona generation failed after {max_attempts} attempts: {last_error}; using rule-based generation")
        return self._generate_profile_rule_based(
            entity_name, entity_type, entity_summary, entity_attributes
        )
    
    def _fix_truncated_json(self, content: str) -> str:
        """Repair JSON truncated by an output token limit."""
        import re
        ...
        # Check for an unclosed string. If the content does not end with a
        # quote, comma, or closing delimiter, the final string may be truncated.
    ...
    def _try_fix_json(self, content: str, entity_name: str, entity_type: str, entity_summary: str = "") -> Dict[str, Any]:
        ...
        if json_match:
            ...
            # 3. Replace newlines inside string values
            ...
            try:
                ...
                # 5. If parsing still fails, use a more aggressive repair
                try:
        ...
        # 6. Extract partial information from the content
        ...
        # 7. Return a basic structure if repair completely fails
    ...
    def _build_individual_persona_prompt(
        ...
        return f"""Generate a detailed social media user persona for the entity, reflecting the available facts as closely as possible.
...
Generate JSON containing these fields:
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        """Build a detailed persona prompt for a group or institution."""
        
        attrs_str = json.dumps(entity_attributes, ensure_ascii=False) if entity_attributes else "None"
        context_str = context[:3000] if context else "No additional context"
        
        return f"""Generate a detailed social media account profile for an institution or group, reflecting the available facts as closely as possible.
...
- The account's statements must be consistent with its institutional role."""
    ...
    def generate_profiles_from_entities(
        ...
        Args:
            ...
            use_llm: Whether to use an LLM for detailed personas.
            progress_callback: Progress callback (current, total, message).
            graph_id: Graph ID used to retrieve richer context from Zep.
            parallel_count: Parallel worker count, default 5.
            realtime_output_path: Optional path updated after each profile.
            output_platform: Output platform format, "reddit" or "twitter".

        Returns:
            The generated agent profiles.
        """
        import concurrent.futures
        ...
        completed_count = [0]  # Use a list so the closure can update it
        lock = Lock()
        
        # Helper for writing profiles as they are generated
        def save_profiles_realtime():
            """Save generated profiles to the output file in real time."""
        ...
        def generate_single_profile(idx: int, entity: EntityNode) -> tuple:
            """Worker that generates one profile."""
            ...
            try:
                ...
                # Print the generated persona in real time
        ...
        logger.info(f"Starting parallel generation of {total} agent personas with {parallel_count} workers")
        print(f"\n{'='*60}")
        print(f"Generating agent personas for {total} entities with {parallel_count} workers")
        print(f"{'='*60}\n")
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=parallel_count) as executor:
            ...
            for future in concurrent.futures.as_completed(future_to_entity):
                ...
                try:
                    ...
                    if error:
                        logger.warning(f"[{current}/{total}] {entity.name} is using a fallback persona: {error}")
                    else:
                    ...
                    # Write updates even for fallback personas
                    save_profiles_realtime()
    ...
    def _print_generated_profile(self, entity_name: str, entity_type: str, profile: OasisAgentProfile):
        """Print the complete generated persona without truncation."""
        separator = "-" * 70
        
        # Build complete output without truncation
        ...
        # Print only to the console to avoid duplicate full-content logs
        print(output)
    ...
    def save_profiles(
        ...
        """Save profiles in the format required by the selected platform.

        Twitter uses CSV and Reddit uses JSON.
    ...
    def _save_twitter_csv(self, profiles: List[OasisAgentProfile], file_path: str):
        """Save Twitter profiles as CSV in the official OASIS format.

        Required fields are user_id, name, username, user_char, and
        description. user_char is the internal persona injected into the LLM
        system prompt; description is the short public biography.
        """
        import csv
        ...
        with open(file_path, 'w', newline='', encoding='utf-8') as f:
            ...
            for idx, profile in enumerate(profiles):
                # user_char: complete persona used by the LLM system prompt
                ...
                # description: short public biography
                ...
                row = [
                    ...
                    user_char,              # user_char: complete internal persona
                    description             # description: short public biography
                ]
                writer.writerow(row)
        
        logger.info(f"Saved {len(profiles)} Twitter profiles to {file_path} in OASIS CSV format")
    
    def _normalize_gender(self, gender: Optional[str]) -> str:
        """Normalize gender to the English values required by OASIS.

        OASIS accepts male, female, and other.
        """
        if not gender:
            return "other"
    ...
    def _save_reddit_json(self, profiles: List[OasisAgentProfile], file_path: str):
        ...
        The format matches to_reddit_format() so OASIS can read it. user_id is
        required for agent_graph.get_agent() to match poster_agent_id values.
        """
        data = []
        for idx, profile in enumerate(profiles):
            # Use the same format as to_reddit_format()
            item = {
                "user_id": profile.user_id if profile.user_id is not None else idx,  # Required user ID
        ...
        logger.info(f"Saved {len(profiles)} Reddit profiles to {file_path} as JSON with user_id fields")
    
    # Preserve the old method name as a backward-compatible alias
