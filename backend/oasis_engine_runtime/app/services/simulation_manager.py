...
Uses preset scripts and LLM-generated configuration parameters.
"""
...
logger = get_logger('rekakebijakan.simulation')
...
class SimulationState:
    ...
    def to_dict(self) -> Dict[str, Any]:
        """Return the full state dictionary for internal use."""
        return {
    ...
    def get_default_platform(self) -> str:
        """Return the default platform based on enabled platforms."""
        if self.enable_twitter and self.enable_reddit:
            return "reddit"  # Preserve the original default when both are enabled.
    ...
    def to_simple_dict(self) -> Dict[str, Any]:
        """Return the simplified state dictionary used by the API."""
        return {
            "simulation_id": self.simulation_id,
            "project_id": self.project_id,
            "graph_id": self.graph_id,
...
class SimulationManager:
    ...
    3. Generate simulation configuration parameters with an LLM.
    ...
    def prepare_simulation(
        ...
        Steps:
        1. Read and filter entities from the Zep graph.
        2. Generate an OASIS agent profile for each entity, optionally enhanced
           by an LLM and generated in parallel.
        3. Generate simulation parameters such as timing, activity, and posting frequency.
        4. Save configuration and profile files.
        5. Copy preset scripts to the simulation directory.
        
        Args:
            simulation_id: Simulation ID.
            simulation_requirement: Requirements used by the LLM to generate configuration.
        ...
        try:
            ...
            if max_profile_count is not None:
                filtered.entities = filtered.entities[:max(1, int(max_profile_count))]
            ...
            # Set the live-save path, preferring Reddit JSON format.
            ...
            profiles = generator.generate_profiles_from_entities(
                ...
                graph_id=state.graph_id,  # Enable Zep retrieval.
                parallel_count=parallel_profile_count,  # Parallel generation count.
                realtime_output_path=realtime_output_path,  # Live-save path.
            ...
            # Save profiles: Twitter uses CSV, while Reddit uses JSON.
            # Reddit profiles were saved during generation; save again for completeness.
            if progress_callback:
                progress_callback(
            ...
            if state.enable_twitter:
                # OASIS requires CSV format for Twitter.
                generator.save_profiles(
                    profiles=profiles,
            ...
            # Runtime scripts remain in backend/scripts/ and are not copied.
            # simulation_runner executes them from scripts/ when the simulation starts.
    ...
    def list_simulations(self, project_id: Optional[str] = None) -> List[SimulationState]:
        ...
        if os.path.exists(self.SIMULATION_DATA_DIR):
            for sim_id in os.listdir(self.SIMULATION_DATA_DIR):
                # Skip hidden entries such as .DS_Store and non-directory files.
    ...
    def get_profiles(self, simulation_id: str, platform: str = None) -> List[Dict[str, Any]]:
        """Return the simulation's agent profiles."""
        state = self._load_simulation_state(simulation_id)
        if not state:
            raise ValueError(f"Simulation does not exist: {simulation_id}")
    ...
    def get_run_instructions(self, simulation_id: str) -> Dict[str, str]:
        """Return simulation run instructions."""
        sim_dir = self._get_simulation_dir(simulation_id)
        config_path = os.path.join(sim_dir, "simulation_config.json")
        ...
        return {
            ...
            "instructions": (
                f"1. Activate the conda environment: conda activate rekakebijakan\n"
