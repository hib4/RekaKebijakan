...
Provides inter-process communication between the Flask backend and simulation scripts.

Implements a simple command/response pattern through the file system:
1. Flask writes commands to the commands/ directory.
2. The simulation script polls for commands, executes them, and writes responses to responses/.
...
logger = get_logger('rekakebijakan.simulation_ipc')


class CommandType(str, Enum):
    """Command types."""
    INTERVIEW = "interview"           # Interview one agent
    BATCH_INTERVIEW = "batch_interview"  # Interview multiple agents
...
class SimulationIPCClient:
    ...
    def send_interview(
        ...
        """
        Send a command to interview one agent.
        
        Args:
            agent_id: Agent ID
            prompt: Interview question.
            platform: Optional target platform.
                - "twitter": Interview the Twitter agent only.
                - "reddit": Interview the Reddit agent only.
                - None: Interview both platforms in a dual-platform simulation,
                  or the active platform in a single-platform simulation.
    ...
    def send_batch_interview(
        ...
        Args:
            interviews: Interview entries containing agent_id, prompt, and an
                optional platform.
            platform: Optional default platform, overridden by each entry's platform.
                ...
                - None: Interview each agent on both platforms in a dual-platform simulation.
            timeout: Timeout duration.
    ...
    def send_close_env(self, timeout: float = 30.0) -> IPCResponse:
        """
        Send a command to close the environment.
    ...
    def check_env_alive(self) -> bool:
        ...
        Determines this by checking the env_status.json file.
        """
...
class SimulationIPCServer:
    ...
    def poll_commands(self) -> Optional[IPCCommand]:
        ...
        # Read command files in chronological order.
        command_files = []
