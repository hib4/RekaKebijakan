...
This script reads parameters from a configuration file and runs a fully automated simulation.

Features:
- Keeps the environment open after simulation and enters command-waiting mode.
...
class UnicodeFormatter(logging.Formatter):
    """Convert Unicode escape sequences into readable characters."""
...
class TwitterSimulationRunner:
    ...
    def __init__(self, config_path: str, wait_for_commands: bool = True):
        ...
        Args:
            ...
            wait_for_commands: Whether to await commands after simulation (default: True).
        """
    ...
    def _get_profile_path(self) -> str:
        """Get the profile path (OASIS Twitter uses CSV format)."""
    ...
    def _create_model(self):
        ...
        Use settings from the project-root .env file with highest priority:
        - LLM_API_KEY: API key
        ...
        # Fall back to config if .env does not provide a model.
        if not llm_model:
    ...
    def _get_active_agents_for_round(
        ...
        # Calculate activation probability from each agent's configuration.
        candidates = []
        for cfg in agent_configs:
            ...
            # Check whether this is an active hour.
            if current_hour not in active_hours:
                continue
            
            # Apply the configured activity probability.
    ...
    async def run(self, max_rounds: int = None):
        ...
        for round_num in range(total_rounds):
            ...
            # Get agents active in this round.
        ...
        if self.wait_for_commands:
            print("\n" + "=" * 60)
            print("Entering command-waiting mode; environment remains active")
...
async def main():
    ...
    parser.add_argument(
        ...
        action='store_true',
        default=False,
        help='Close the environment immediately after simulation without waiting for commands'
    )
    ...
    # Initialize logging with fixed file names and remove old logs.
...
def setup_signal_handlers():
    ...
    Give the program an opportunity to clean up resources such as the database and environment.
    """
