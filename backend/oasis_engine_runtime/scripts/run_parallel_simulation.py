...
Runs Twitter and Reddit simulations concurrently from the same configuration file.

Features:
- Runs Twitter and Reddit simulations in parallel.
- Keeps environments open after simulation and enters command-waiting mode.
- Receives interview commands through IPC.
- Supports individual and batch agent interviews.
...
Log structure:
    ...
    └── run_state.json       # Runtime status (used by API queries)
"""

# ============================================================
# Address Windows encoding issues by setting UTF-8 before all imports.
# This fixes third-party OASIS libraries that read files without specifying an encoding.
# ============================================================
import sys
import os
...
if sys.platform == 'win32':
    ...
    # Reconfigure standard streams as UTF-8 to prevent garbled console text.
    ...
    # This normally must be set when Python starts and may not take effect at runtime,
    # so the built-in open function is also monkey-patched.
    import builtins
    ...
    def _utf8_open(file, mode='r', buffering=-1, encoding=None, errors=None, 
        ...
        """
        Wrap open() to default to UTF-8 in text mode.
        This fixes third-party libraries such as OASIS that omit an encoding when reading files.
        """
        # Set the default only for text mode when no encoding was specified.
...
# This script has a fixed location under backend/scripts/.
...
class MaxTokensWarningFilter(logging.Filter):
    """Filter camel-ai max_tokens warnings because the model chooses its own limit."""
    
    def filter(self, record):
        # Filter log records containing the max_tokens warning.
        if "max_tokens" in record.getMessage() and "Invalid or missing" in record.getMessage():
            return False
        return True


# Add the filter at module load so it takes effect before camel code runs.
...
def disable_oasis_logging():
    ...
    OASIS logs every agent observation and action, so use the custom action_logger instead.
    """
    ...
    for logger_name in oasis_loggers:
        ...
        logger.setLevel(logging.CRITICAL)  # Record critical errors only.
...
def init_logging_for_simulation(simulation_dir: str):
    """
    Initialize logging for a simulation.
...
# Available Reddit actions. INTERVIEW can only be triggered manually via ManualAction.
REDDIT_ACTIONS = [
    ActionType.LIKE_POST,
...
class ParallelIPCHandler:
    ...
    def poll_command(self) -> Optional[Dict[str, Any]]:
        ...
        # Get command files in chronological order.
        command_files = []
    ...
    async def _interview_single_platform(self, agent_id: int, prompt: str, platform: str) -> Dict[str, Any]:
        ...
        Returns:
            A dictionary containing either the result or an error.
        """
    ...
    async def handle_interview(self, command_id: str, agent_id: int, prompt: str, platform: str = None) -> bool:
        """
        Handle an individual agent interview command.
        
        Args:
            command_id: Command ID.
            agent_id: Agent ID
            prompt: Interview question.
            platform: Optional platform selection.
                - "twitter": Interview on Twitter only.
                - "reddit": Interview on Reddit only.
                - None/unspecified: Interview on both platforms and combine the results.
            
        Returns:
            True on success; False on failure.
        """
        # Interview only the selected platform when specified.
        ...
        # If no platform is specified, interview on both platforms.
        if not self.twitter_env and not self.reddit_env:
            self.send_response(command_id, "failed", error="No simulation environments are available")
            return False
        ...
        # Interview on both platforms concurrently.
        tasks = []
    ...
    async def handle_batch_interview(self, command_id: str, interviews: List[Dict], platform: str = None) -> bool:
        ...
        Args:
            ...
            platform: Default platform, which each interview item may override.
                - "twitter": Interview on Twitter only.
                - "reddit": Interview on Reddit only.
        ...
        twitter_interviews = []
        reddit_interviews = []
        both_platforms_interviews = []  # Interviews that must run on both platforms.
        ...
        # Add dual-platform interviews to both platform groups.
...
# Non-core action types to filter because they provide little analytical value.
...
def get_agent_names_from_config(config: Dict[str, Any]) -> Dict[int, str]:
    ...
    This displays actual entity names in actions.jsonl instead of aliases such as "Agent_0".
...
def fetch_new_actions_from_db(
    ...
    """
    Get new action records from the database and add full context.
    
    Args:
        db_path: Database file path.
        last_rowid: Largest rowid previously read. rowid is used because created_at formats differ by platform.
        agent_names: agent_id -> agent_name mapping.
        
    Returns:
        (actions_list, new_last_rowid)
        - actions_list: Actions containing agent_id, agent_name, action_type, and contextual action_args.
    ...
    try:
        ...
        # Track processed records with SQLite's built-in, auto-incrementing rowid.
        # This avoids created_at differences: Twitter uses integers and Reddit uses datetime strings.
        cursor.execute("""
        ...
        for rowid, user_id, action, info_json in cursor.fetchall():
            ...
            # Keep only key action_args fields without truncating their content.
            simplified_args = {}
            ...
            # Convert the action type name.
            action_type = ACTION_TYPE_MAP.get(action, action.upper())
            
            # Add context such as post content and user names.
...
def _enrich_action_context(
    ...
    """
    Add context such as post content and user names to an action.
    ...
    try:
        ...
        # Repost: add original post content and author.
            ...
            if new_post_id:
                # A repost's original_post_id points to the original post.
                cursor.execute("""
        ...
        # Quote post: add original content, author, and quote comment.
        ...
        # Create comment: add information about the commented post.
        ...
        # Context enrichment failures do not affect the main flow.
...
def _get_post_info(
    ...
    try:
        ...
        if row:
            ...
            # Prefer the name in agent_names.
            author_name = ''
...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    ...
    Supports two LLM configurations to accelerate parallel simulation:
    - General configuration: LLM_API_KEY, LLM_BASE_URL, LLM_MODEL_NAME
    - Optional boost configuration: LLM_BOOST_API_KEY, LLM_BOOST_BASE_URL, LLM_BOOST_MODEL_NAME
    
    A configured boost LLM lets each platform use a different API provider for greater concurrency.
    
    Args:
        config: Simulation configuration dictionary.
        use_boost: Whether to use the boost LLM configuration when available.
    """
    # Check for a boost configuration.
    boost_api_key = os.environ.get("LLM_BOOST_API_KEY", "")
    boost_base_url = os.environ.get("LLM_BOOST_BASE_URL", "")
    boost_model = os.environ.get("LLM_BOOST_MODEL_NAME", "")
    has_boost_config = bool(boost_api_key)
    
    # Select the LLM based on the arguments and available configuration.
    ...
    # Fall back to config if .env does not provide a model name.
    if not llm_model:
...
async def run_twitter_simulation(
    ...
    Args:
        ...
        max_rounds: Optional maximum round count used to truncate long simulations.
        
    Returns:
        PlatformSimulation containing the environment and agent graph.
    """
    ...
    # Use the OASIS default name when an agent is absent from the configuration.
    ...
    total_actions = 0
    last_rowid = 0  # Track the last processed row with rowid to avoid created_at format differences.
    
    # Run initial events.
    event_config = config.get("event_config", {})
    initial_posts = event_config.get("initial_posts", [])
    
    # Record the start of round 0, the initial-event phase.
    if action_logger:
    ...
    for round_num in range(total_rounds):
        ...
        # Record the round start whether or not any agents are active.
        if action_logger:
            action_logger.log_round_start(round_num + 1, simulated_hour)
        
        if not active_agents:
            # Record the round end with actions_count=0 when no agents are active.
            if action_logger:
    ...
    # Keep the environment open for interviews.
    ...
    log_info(f"Simulation loop complete! Elapsed: {elapsed:.1f} seconds, total actions: {total_actions}")
...
async def run_reddit_simulation(
    ...
    # Reddit uses the boost LLM configuration when available, otherwise the general configuration.
...
async def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help='Optional maximum round count for truncating long simulations'
    )
    parser.add_argument(
        '--no-wait',
        action='store_true',
        default=False,
        help='Close environments immediately after simulation without waiting for commands'
    )
    
    args = parser.parse_args()
    
    # Create the shutdown event at the start of main so the entire program can respond to signals.
    ...
    # Initialize logging, disable OASIS logs, and remove old files.
    ...
    if args.twitter_only:
        ...
        # Run in parallel with a separate logger for each platform.
    ...
    log_manager.info("=" * 60)
    log_manager.info(f"Simulation loops complete! Total elapsed: {total_elapsed:.1f} seconds")
    
    # Enter command-waiting mode if requested.
    if wait_for_commands:
        log_manager.info("")
        log_manager.info("=" * 60)
        log_manager.info("Entering command-waiting mode; environments remain active")
        ...
        # Command-waiting loop using the global _shutdown_event.
        try:
...
def setup_signal_handlers(loop=None):
    """
    Set up signal handlers to exit correctly on SIGTERM or SIGINT.
    
    Persistent simulation mode waits for interview commands after simulation.
    On a termination signal:
    1. Notify the asyncio loop to stop waiting.
    2. Allow normal cleanup of resources such as databases and environments.
    3. Exit afterward.
    """
    def signal_handler(signum, frame):
        ...
        if not _cleanup_done:
            _cleanup_done = True
            # Notify the asyncio loop so it can exit and clean up resources.
            if _shutdown_event:
                _shutdown_event.set()
        
        # Do not call sys.exit() directly; let the asyncio loop exit and clean up.
        # Force exit only after receiving another signal.
        else:
            print("Forcing exit...")
            sys.exit(1)
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Stop the multiprocessing resource tracker to prevent warnings on exit.
        try:
