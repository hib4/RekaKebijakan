...
# Capture the caller-supplied key before importing rekakebijakan modules. app.config
...
BASELINE_EPISODES = [
    SourceEpisode(
        ...
        "Lanzhou Technology (stable company ID LZ-TECH) provides intelligent wind-power operations and maintenance. "
        "The company is headquartered in Xinggang City, and Zhou Lan has served as CEO since 2024.",
        "baseline",
    ),
    SourceEpisode(
        "2026-01-12T10:00:00Z",
        "Lanzhou Technology developed the Zhixun Platform (stable product ID ZHIXUN-01). "
        "The Zhixun Platform is currently in the pilot stage and identifies wind-turbine blade anomalies.",
        "baseline",
    ),
    SourceEpisode(
        "2026-01-20T14:00:00Z",
        "Lanzhou Technology and Haiyue Energy (stable company ID HY-ENERGY) signed partnership agreement HY-2026-01. "
        "They will jointly deploy the Zhixun Platform at Dongwan Wind Farm; the agreement is valid through December 31, 2026.",
        "baseline",
    ),
    SourceEpisode(
        "2026-02-02T09:30:00Z",
        "Chen Yu is the project lead for Lanzhou Technology's Zhixun Platform and is responsible for the Dongwan Wind Farm deployment. "
        "CEO Zhou Lan remains responsible for company operations.",
        "baseline",
    ),
    SourceEpisode(
        "2026-02-20T18:00:00Z",
        "The Zhixun Platform pilot at Dongwan Wind Farm detected 12 blade anomalies, "
        "reducing Haiyue Energy's unplanned downtime by 18%.",
        "baseline",
    ),
    SourceEpisode(
        "2026-03-05T11:00:00Z",
        "Haiyue Energy confirmed that Lanzhou Technology develops the Zhixun Platform and Chen Yu leads implementation. "
        "Haiyue Energy plans to become one of its first commercial customers after pilot acceptance.",
        "baseline",
    ),
    SourceEpisode(
        "2026-03-15T16:00:00Z",
        "Lanzhou Technology's board approved moving the Zhixun Platform from pilot to commercial release on April 1, 2026. "
        "Zhou Lan signed the release resolution as CEO.",
        "baseline",
    ),
    SourceEpisode(
        "2026-04-01T08:00:00Z",
        "The Zhixun Platform was commercially released today and is no longer in the pilot stage. "
        "Haiyue Energy became its first commercial customer, and Chen Yu remains responsible for delivery.",
        "baseline",
    ),
    SourceEpisode(
        "2026-04-18T13:00:00Z",
        "Lanzhou Technology announced at its Xinggang City headquarters that the Zhixun Platform now covers 60 wind turbines. "
        "Haiyue Energy remains a partner and customer under agreement HY-2026-01.",
        "baseline",
    ),
    SourceEpisode(
        "2026-04-30T17:00:00Z",
        "As of April 30, 2026, Zhou Lan remains CEO of Lanzhou Technology, "
        "which remains headquartered in Xinggang City; Chen Yu leads the Zhixun Platform project.",
        "baseline",
    ),
]
...
TEMPORAL_UPDATES = [
    SourceEpisode(
        ...
        "Lanzhou Technology's board announced that Zhou Lan ceased serving as CEO on May 10, 2026. "
        "Chen Yu formally succeeded Zhou Lan as CEO, and Zhou Lan became chief strategy adviser.",
        "leadership_change",
    ),
    SourceEpisode(
        "2026-06-01T09:00:00Z",
        "On June 1, 2026, Lanzhou Technology moved its headquarters from Xinggang City to Haicheng City. "
        "The former Xinggang City headquarters is now Lanzhou Technology's research and development center.",
        "headquarters_change",
    ),
    SourceEpisode(
        "2026-06-15T12:00:00Z",
        "Lanzhou Technology and Haiyue Energy terminated joint deployment agreement HY-2026-01 early on June 15, 2026. "
        "Haiyue Energy is no longer Lanzhou Technology's joint deployment partner but remains a Zhixun Platform customer.",
        "partnership_change",
    ),
    SourceEpisode(
...
def _episode_to_batch_item(graph_id: str, item: SourceEpisode, index: int) -> BatchAddItem:
    return BatchAddItem(
        ...
        source_description="rekakebijakan deep Zep Cloud validation corpus",
        metadata={
            "source": "rekakebijakan_zep_deep_validation",
            "phase": item.phase,
            "sequence": index,
        },
    )
...
def _add_and_wait(client: Any, graph_id: str, item: SourceEpisode, timeout: int) -> str:
    episode = client.graph.add(
        ...
        data=item.data,
        created_at=item.created_at,
        source_description="rekakebijakan temporal Zep Cloud validation update",
        metadata={"source": "rekakebijakan_zep_deep_validation", "phase": item.phase},
    )
...
def _activities() -> Iterable[AgentActivity]:
    return [
        AgentActivity(
            ...
            action_args={"content": "The new Haicheng City headquarters opened today, and Zhixun Platform commercial services are operating normally."},
            round_num=1,
        ...
        AgentActivity(
            ...
            action_args={
                ...
                "original_content": "The new Haicheng City headquarters opened today, and Zhixun Platform commercial services are operating normally.",
        ...
        AgentActivity(
            ...
            action_args={
                ...
                "post_content": "The new Haicheng City headquarters opened today, and Zhixun Platform commercial services are operating normally.",
            },
            round_num=1,
        ...
        AgentActivity(
            ...
            action_args={
                "post_author_name": "Chen Yu",
                "post_content": "The new Haicheng City headquarters opened today, and Zhixun Platform commercial services are operating normally.",
                "content": "The joint deployment agreement has ended, but we remain a Zhixun Platform customer.",
            },
            round_num=2,
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    graph_id = args.graph_id or f"rekakebijakan_zep_deep_{stamp}"
    ...
    try:
        builder.create_graph("rekakebijakan Zep Cloud Deep Validation", graph_id=graph_id)
        created = True
        ...
        try:
            ...
            result["empty_attribute_ontology_probe"] = "accepted_after_rekakebijakan_normalization"
        except Exception as error:
        ...
        batch = client.batch.create(
            metadata={
                "rekakebijakan_operation_id": operation_id,
                "graph_id": graph_id,
        ...
        baseline_ceo = client.graph.search(
            graph_id=graph_id,
            query="Who was CEO of Lanzhou Technology at the end of April 2026?",
            scope="edges",
        ...
        if updater_stats["items_sent"] != 5 or updater_stats["pending_episode_count"] != 0:
            raise AssertionError(f"unexpected rekakebijakan updater stats: {updater_stats}")
        result["rekakebijakan_updater"] = updater_stats
        print("[zep-deep] rekakebijakan updater processed 5 mock activities", flush=True)
        ...
        edge_search = client.graph.search(
            graph_id=graph_id,
            query="Who is the current CEO of Lanzhou Technology, where is its current headquarters, and what is its current relationship with Haiyue Energy?",
            scope="edges",
            reranker="cross_encoder",
            limit=20,
        )
        node_search = client.graph.search(
            graph_id=graph_id,
            query="Lanzhou Technology management personnel",
            scope="nodes",
            reranker="rrf",
            limit=10,
        ...
        typed_edge_search = client.graph.search(
            graph_id=graph_id,
            query="What changes occurred in company management roles?",
            scope="edges",
            reranker="rrf",
            limit=10,
        ...
        auto_search = client.graph.search(
            graph_id=graph_id,
            query="Summarize Lanzhou Technology's latest management, headquarters, product, and relationship with Haiyue Energy.",
            scope="auto",
            max_characters=3000,
        ...
        if len(entity_context.related_edges) != len(complete_node_edges):
            raise AssertionError(
                "rekakebijakan entity context omitted incoming or outgoing node edges"
            )
...
def _compact_result(result: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "rekakebijakan_updater": result.get("rekakebijakan_updater"),
        "final": {
            key: final.get(key)
            for key in (
                "node_count",
