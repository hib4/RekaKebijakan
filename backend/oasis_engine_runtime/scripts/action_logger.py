"""
Action logger
Records every agent action in an OASIS simulation for backend monitoring.
...
class SimulationLogManager:
    ...
    Manages all log files centrally while separating them by platform.
    """
...
class ActionLogger:
    """
    Action logger compatible with the legacy interface.
...
# Global logger instance for legacy interface compatibility.
_global_logger: Optional[ActionLogger] = None


def get_logger(log_path: Optional[str] = None) -> ActionLogger:
    """Get the global logger instance for legacy interface compatibility."""
