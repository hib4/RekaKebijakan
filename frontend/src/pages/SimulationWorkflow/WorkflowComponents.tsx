...
export function WorkflowTopBar({ session, onStep, onViewMode, connectionStatus, connectionLabel }: { session: WorkflowSession; onStep: (step: WorkflowStep) => void; onViewMode: (mode: ViewMode) => void; connectionStatus: string; connectionLabel: string }) {
  ...
  return <>
    <header className="workflow-topbar">
      ...
      <div className="view-modes" aria-label="Mode tampilan">{(["graph", "split", "workbench"] as ViewMode[]).map((mode) => <button key={mode} aria-pressed={effectiveViewMode === mode} disabled={session.currentStep >= 4 && mode !== "workbench"} onClick={() => onViewMode(mode)}>{mode === "graph" ? "Graf" : mode === "split" ? "Terbagi" : "Ruang kerja"}</button>)}</div>
