...
export function PolicyGraph({ demo, nodeCount, edgeCount, activeNodeId, selectedNodeId, onSelect, onLog, graphLabel = "POLICY KNOWLEDGE GRAPH" }: { demo: DemoCase; nodeCount: number; edgeCount: number; activeNodeId: string | null; selectedNodeId: string | null; onSelect: (id: string | null) => void; onLog: (message: string) => void; graphLabel?: string }) {
  ...
  return <section className="policy-graph-panel" aria-labelledby="graph-title"><header className="graph-toolbar"><div><p>{graphLabel}</p><h2 id="graph-title">{demo.title}</h2></div><div className="graph-actions"><button onClick={() => { setLayoutKey((value) => value + 1); onLog("Graph layout refreshed"); }}>↻ <span>Refresh</span></button><button onClick={fit}>⊙ <span>Fit</span></button></div></header><div className="graph-filters"><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Cari entity..." aria-label="Cari entity graph" /><select value={filter} onChange={(event) => setFilter(event.target.value)} aria-label="Filter tipe entity"><option>Semua</option>{graphTypes.map((type) => <option key={type}>{type}</option>)}</select><label><input type="checkbox" checked={showLabels} onChange={(event) => setShowLabels(event.target.checked)} /> Label relasi</label></div><div className="graph-stage">
    ...
    {selected && <aside className="graph-inspector" aria-label="Detail entity"><div><span style={{ background: nodeColor(selected.type) }} />{selected.type}</div><h3>{selected.label}</h3><p>{selected.summary}</p>{selected.group && <dl><dt>Kelompok</dt><dd>{selected.group}</dd></dl>}<CitationDrawer citations={selected.citations} label="Lihat sumber entity" /><button onClick={() => onSelect(null)}>Tutup detail</button></aside>}
    <div className="graph-legend" aria-label="Legenda entity">{graphTypes.map((type) => <button key={type} className={filter === type ? "active" : ""} onClick={() => setFilter(filter === type ? "Semua" : type)}><i style={{ background: nodeColor(type) }} />{type}</button>)}</div>
  </div></section>;
}
