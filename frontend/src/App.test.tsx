...
describe("application routing", () => {
  it("redirects an unauthenticated protected route and preserves the destination", async () => {
    ...
    expect(await screen.findByRole("heading", { name: "Masuk ke RekaKebijakan" })).toBeInTheDocument();
...
describe("project detail resources", () => {
  it("loads the route project and creates a server-backed scenario", async () => {
    ...
    server.use(
      http.get("/backend/api/auth/me", () => HttpResponse.json({ user: { id: "user-1", name: "Analis", email: "analis@example.com" } })),
      http.get("/backend/api/v1/projects/project-1", () => HttpResponse.json({
        id: "project-1",
        name: "Program Nyata",
        ...
        archived_at: null,
        documents: [{ id: "doc-1", name: "policy.pdf", media_type: "application/pdf" }],
        snapshot: {
      ...
      http.get("/backend/api/v1/projects/project-1/scenarios", () => HttpResponse.json({ items: scenarios })),
      http.post("/backend/api/v1/projects/project-1/scenarios", async ({ request }) => {
    ...
    expect(await screen.findByRole("heading", { level: 1, name: "Program Nyata" })).toBeInTheDocument();
    expect(screen.getByText("policy.pdf")).toBeInTheDocument();
    expect(screen.getAllByText("Bagaimana dampaknya?")).toHaveLength(2);

    await user.type(screen.getByLabelText("Nama skenario"), "Sosialisasi intensif");
    await user.type(screen.getByLabelText("Deskripsi"), "Bandingkan kanal pendampingan");
    await user.click(screen.getByRole("button", { name: "Tambah skenario" }));

    expect(await screen.findByText("Sosialisasi intensif", { selector: "b" })).toBeInTheDocument();
    expect(screen.getByText("Skenario Sosialisasi intensif tersimpan.")).toBeInTheDocument();
  });
});
